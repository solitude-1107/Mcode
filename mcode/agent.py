...
from mcode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from mcode.conversation import ThinkingBlock as ConvThinkingBlock
from mcode.memory.auto_memory import MemoryManager
from mcode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from mcode.hooks import HookContext, HookEngine, ToolRejectedError
from mcode.hooks.engine import HookNotification
from mcode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from mcode.tools import ToolRegistry
from mcode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".mcode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".mcode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from mcode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from mcode.context.manager import (
