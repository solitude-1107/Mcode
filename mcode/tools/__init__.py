...
from mcode.tools.base import Tool
...
class ToolRegistry:
    def __init__(self) -> None:
        self._tools: dict[str, Tool] = {}   #直接加载的工具
        self._disabled: set[str] = set()    #禁用的工具
        self._discovered: set[str] = set()  #加载的延迟加载的工具
...
def create_default_registry(file_cache: FileCache | None = None, file_history: Any = None) -> ToolRegistry:
    from mcode.tools.bash import Bash
    from mcode.tools.edit_file import EditFile
    from mcode.tools.file_state_cache import FileStateCache
    from mcode.tools.glob import Glob
    from mcode.tools.grep import Grep
    from mcode.tools.read_file import ReadFile
    from mcode.tools.write_file import WriteFile
