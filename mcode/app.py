...
from mcode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from mcode.commands.completion import CompletionPopup
from mcode.commands.handlers import register_all_commands
from mcode.config import MCPServerConfig, ProviderConfig
from mcode.hooks import HookContext, HookEngine, load_hooks
from mcode.conversation import ConversationManager, Message
from mcode.mcp import ConnectResult, MCPManager
from mcode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from mcode.agents.task_manager import TaskManager
from mcode.agents.trace import TraceManager
from mcode.agents.notification import inject_task_notifications
from mcode.commands.handlers.tasks import create_tasks_command
from mcode.skills.executor import SkillExecutor
from mcode.skills.loader import SkillLoader
from mcode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from mcode.cache import FileCache
from mcode.tools import ToolRegistry, create_default_registry
from mcode.tools.agent_tool import AgentTool
from mcode.tools.ask_user import AskUserEvent, AskUserTool
from mcode.tools.impl.tool_search import ToolSearchTool
from mcode.tools.install_skill import InstallSkillTool
from mcode.tools.load_skill import LoadSkill
from mcode.worktree.cleanup import start_stale_cleanup_task
from mcode.worktree.manager import WorktreeManager
from mcode.commands.handlers.worktree import create_worktree_command
from mcode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".mcode", "build", ".gradle"}
...
class ChatInput(TextArea):
    BINDINGS = [
        ...
        Binding("escape", "dismiss_popup", "关闭", priority=True),
        Binding("up", "nav_up", "上移", priority=True),
        Binding("down", "nav_down", "下移", priority=True),
    ]
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".mcode" / "history"
...
class MCodeApp(App):
    ...
    BINDINGS = [
        ...
        Binding("shift+tab", "cycle_mode", "切换模式", priority=True),
        Binding("ctrl+o", "toggle_tool_blocks", "切换工具", priority=True),
    ]
    ...
    def __init__(
        ...
        from mcode.config import SandboxAppConfig
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        ...
        t.append("  ███   ███   ███  \n", style="bold color(208)")
        t.append("  ███   ███   ███  ", style="bold color(208)")
        t.append("MCode v26.08.22\n", style="color(242)")
        t.append("  ███   ███   ███  ", style="bold color(208)")
        if model:
            t.append(f"{model}\n", style="color(242)")
        else:
    ...
    def compose(self) -> ComposeResult:
        ...
        if len(self.providers) > 1:
            with Vertical(id="provider-select"):
                yield Static("选择服务商", id="select-label")
                yield OptionList(
                    *[
    ...
    def on_mount(self) -> None:
        self.register_theme(_MCODE_THEME)
        self.theme = "mcode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".mcode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".mcode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".mcode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from mcode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.mcode/config.yaml",
                        f"{work_dir}/.mcode/permissions.local.yaml",
                    ],
        ...
        from mcode.filehistory import FileHistory
        ...
        from mcode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from mcode.config import WorktreeConfig
        ...
        from mcode.tools.enter_worktree import EnterWorktreeTool
        from mcode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from mcode.teams.manager import TeamManager
        from mcode.tools.team_create import TeamCreateTool
        from mcode.tools.team_delete import TeamDeleteTool
        ...
        from mcode.commands.handlers.trace import create_trace_command
        ...
        from mcode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def on_chat_input_submitted(self, event: ChatInput.Submitted) -> None:
        ...
        if self._streaming and not text.startswith("/"):
            ...
            self._show_system_message("（响应已中断）")
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from mcode.tools.base import StreamEnd, TextDelta
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        ...
        if self._mcp_init_task and not self._mcp_init_task.done():
            self._show_system_message("等待 MCP 服务器连接...")
        ...
        try:
            async for event in self.agent.run(self.conversation):
                if isinstance(event, ThinkingText):
                    ...
                    self._show_system_message(f"↻ 重试中: {event.reason}")
                    ...
                    done_label = Static(
                        f"✻ {_to_past_tense(self._thinking_verb)}，耗时 {total_time:.1f}秒",
    ...
    async def _show_plan_approval(self) -> None:
        from mcode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from mcode.plan_dialog import InlinePlanWidget, PlanChoice
        from mcode.prompts import build_plan_mode_exit_reminder
        ...
        if choice == PlanChoice.YOLO:
            ...
            if feedback:
                ...
                self._show_system_message("输入你的反馈并发送。")

    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from mcode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from mcode.permission_dialog import InlinePermissionWidget
    ...
    async def _init_mcp(self) -> None:
        ...
        for err in connect_result.errors:
            self._show_system_message(f"MCP 警告: {err}")
