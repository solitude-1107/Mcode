...
from mcode.config import ConfigError, load_config
from mcode.hooks import HookConfigError, HookEngine, load_hooks
from mcode.permissions import PermissionMode


def main() -> None:
    # 先确保 .mcode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".mcode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".mcode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="mcode", description="MCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from mcode.remote import RemoteServer
    ...
    from mcode.app import MCodeApp
    from mcode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from mcode.client import create_client, resolve_context_window
    from mcode.conversation import ConversationManager
    from mcode.memory.instructions import load_instructions
    ...
    from mcode.tools import create_default_registry
