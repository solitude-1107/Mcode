...
from mcode.worktree.models import WorktreeSession
...
def _session_path(mcode_dir: Path) -> Path:
    return mcode_dir / SESSION_FILENAME
...
def load_worktree_session(mcode_dir: Path) -> WorktreeSession | None:
    path = _session_path(mcode_dir)
    if not path.exists():
        return None
    try:
